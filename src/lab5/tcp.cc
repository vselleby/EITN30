	...
	udword theSynchronizationNumber)
{
	...
	case 7:
		...
		TCP::instance().acceptConnection(theConnection->myPort);
...
void TCPInPacket::decode()
{
	...
	else
	{
		...
		//FIN+ACK FLAG
		if ((aTCPHeader->flags == 0x11) && aConnection->myState == FinWait1State::instance())
		{
			aConnection->Acknowledge(myAcknowledgementNumber);
			aConnection->NetClose();
		}
		//FIN FLAG
		else if ((aTCPHeader->flags & 0x01) != 0 && aConnection->receiveNext == mySequenceNumber)
		{
			aConnection->NetClose();
		}
		//PSH FLAG
